...
func (g Galleries) UploadImage(w http.ResponseWriter, r *http.Request) {
	...
	err = r.ParseMultipartForm(5 << 20) //5 mB - как я понял, макс размер файла
	if err != nil {
		http.Error(w, "Something went wrong", http.StatusInternalServerError)
		return
	}
...
// для того, чтобы воспользваться userMustOwnGallery
type galleryOpt func(w http.ResponseWriter, r *http.Request, gallery *models.Gallery) error

// дополнительный метод, чтобы не повторять код в коде )))
func (g Galleries) galleryByID(w http.ResponseWriter, r *http.Request, opts ...galleryOpt) (*models.Gallery, error) {
	...
	if err != nil {
		if errors.Is(err, models.ErrNoFound) {
			http.Error(w, "Galary not found", http.StatusNotFound)
			return nil, err
		}
		http.Error(w, "Something went wrong", http.StatusInternalServerError)
		return nil, err
